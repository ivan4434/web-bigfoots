<template>
  <div class="fixed w-[100%] h-[5rem] flex flex-row items-center duration-200 z-index-50" :class="{'bg-gray-200':scrollPosition > 100, 'bg-[rgba(0,0,0,0)]':scrollPosition <= 100}">
    ...
    <div v-if="windowWidth > 600" class="absolute top-[50%] right-[1rem] translate-y-[-50%] flex flex-row justify-center">
  ...
  <div id="download" class="bg-orange-300 w-[100%] pt-[5rem] flex flex-row items-center">
    <img v-if="windowWidth > 1100" class="w-[50%]" src="https://lh4.googleusercontent.com/7NGiuwUO2K1TN3zmO1hKwZEP7mc2VWiVJWycJunpwCRsakqvI3o3RePY1487vPK0o1IJLy4SXpd54evLEpw7uNEMxYWga29dDjOsJdZkETFJgfNeJUgk-Bz5ZCzF4Uz_Fw=w992" alt="">
    <div :class="{'w-[100%]': windowWidth <= 1100, 'w-[50%]': windowWidth > 1100}" class="flex flex-col items-center justify-center text-center">
      <div class="text-[3rem] font-bold">Big Foots Simulator</div>
      <div class="text-justify text-[.8rem] px-[5rem] py-[1rem]">El simulador de BigFoots más avanzado del mundo, con los mejores gráficos y experiencia inmersiva del mercado. Este juego te sumerge en una emocionante lucha por la supervivencia contra criaturas misteriosas en un vasto mundo abierto. Explora el entorno lleno de secretos y peligros mientras desentrañas el misterio detrás de los Bigfoots. La avanzada inteligencia artificial y fotorrealismo gráfico hacen que cada partida sea única y desafiante, llevándote a vivir una experiencia sin igual.</div>
      <button @click="openTab('https://uvaes-my.sharepoint.com/:u:/g/personal/daniel_garcia22_estudiantes_uva_es/ERMgPE62p7REpJ4pmbrhzW8BjfWSk-0pA8t-BKXcpzYWpw?e=Ca4qwL')" class="mb-[1rem] bg-white px-[1rem] py-[.5rem] rounded-full font-medium mt-[.5rem] font-[1.3rem] hover:bg-transparent border-2 hover:border-blue-400 duration-200 hover:text-blue-400 active:text-blue-300 active:border-blue-300">Descargar</button>
    </div>
  </div>
  ...
  <div id="images" class="w-[100%] flex flex-col p-[3rem]">
    ...
    <div class="flex flex-wrap w-[100%] mt-[1rem] items-center justify-center ">
      <img v-for="(img, index) in images" :class="{'w-[30%]': windowWidth > 800, 'w-[100%]': windowWidth <= 800}" :src="img" :key="index" @click="currentImage = {active:true, img}" class="m-[.1rem] hover:opacity-[70%] duration-200 cursor-pointer active:opacity-[60%] z-index-0" alt="">
    </div>
  </div>
